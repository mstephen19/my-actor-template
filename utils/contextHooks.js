...
const createKVContext = async (state, userReducer) => {
    ...
    if (!check) {
        ...
        runContext = { state, stringReducer: stringReducer() };
        return Apify.setValue('CONTEXT', { state, stringReducer: stringReducer() });
    }
...
const useKVContext = async () => {
    try {
        // Pull from KVStore
        const { state, stringReducer } = await Apify.getValue('CONTEXT');
        runContext = state;
        ...
        const dispatch = async (action) => {
            ...
            await Apify.setValue('CONTEXT', { state: newState, stringReducer });
            runContext = newState;
        };
